...
export function longform(doc, debug = () => {
  ...
  let foundRoot = false, skipping = false, verbatimIndent = null, verbatimFirst = false, element = makeElement(), chunk = makeChunk(), fragment = makeFragment();
  ...
  while (m1 = sniffTestRe.exec(doc)) {
    ...
    switch ((_b = (_a = m1[2]) != null ? _a : m1[3]) != null ? _b : m1[4]) {
      ...
      default: {
        const m22 = text1.exec(m1[0]);
        if (m22 == null) {
          break;
        }
        const indent = m22[1].length / 2;
        debug(indent, "t", m22[2]);
        if (element.tag != null) {
          applyIndent(indent);
          fragment.html += m22[2].trim();
        } else {
          fragment.html += " " + m22[2].trim();
        }
        break;
